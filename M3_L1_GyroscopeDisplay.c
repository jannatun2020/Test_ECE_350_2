 ...
 ***************************************************
 *
 * FILE NAME  :  M3_L1_GyroscopeDisplay.c
...
int initI2C(void)  // sets up both I2C Busses
{
	int status;
	mraa_init();
	...
	if (status < 0){
         	printf("Failed to Detect Accelorometer -> Exiting program\n");
		printf("Check i2cdetect -r -y 2 for 0x6B\n");
         	mraa_i2c_stop(I2Chandle);
         	mraa_deinit();
         	return EXIT_FAILURE;
        }
...
int main(void) {
	...
	if (status != MRAA_SUCCESS){
              printf("Failed to Initialize Gyroscope -> Exiting program %d\n",status);
              mraa_i2c_stop(I2Chandle);
              mraa_deinit();
              return EXIT_FAILURE;
             }
	...
	status = mraa_i2c_read_byte(I2Chandle);
	if (status < 0){
              printf("Failed to Initialize Gyro -> Exiting program %d\n",status);
              mraa_i2c_stop(I2Chandle);
              mraa_deinit();
              return EXIT_FAILURE;
             }
